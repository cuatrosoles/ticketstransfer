...
export function eventDateForApi(value: string): string {
...
export function eventDateDateOnly(value: string): string {
  const d = parseDatetimeLocalValue(value);
  if (!d) {
    const s = value.trim();
...
export function formatEventDateTimeDisplay(value: string, locale = 'es-AR'): string {
  const d = parseDatetimeLocalValue(value);
  if (!d) return value.trim() || 'Seleccionar fecha y hora';
...
export function listingValueToDatetimeLocal(v: unknown): string {
  if (v == null) return '';
  ...
  if (typeof v === 'string') {
    ...
    d = new Date((v as { _seconds: number })._seconds * 1000);
