...
var init_constants = __esm({
  "../../packages/shared/src/constants.ts"() {
    "use strict";
    CATEGORIAS_EVENTOS = ["MUSICA", "DEPORTES", "TEATRO", "FESTIVALES", "OTRO"];
    TICKET_LISTING_STATUS = [
      "BORRADOR",
...
var PREFERENCIAS_EVENTO, PREFERENCIAS_EVENTO_IDS, INTERACTION_WEIGHTS, PREF_TO_LISTING;
var init_user_preferences = __esm({
  "../../packages/shared/src/user-preferences.ts"() {
    ...
    PREFERENCIAS_EVENTO_IDS = PREFERENCIAS_EVENTO.map((p) => p.id);
    INTERACTION_WEIGHTS = {
      VIEW: 1,
      CLICK: 2,
      FAVORITE_ADD: 5,
...
var init_event_images = __esm({
  "../../packages/shared/src/event-images.ts"() {
    "use strict";
    init_constants();
    EVENT_IMAGE_CATEGORY_FALLBACKS = {
...
function distanceKmFrom(originLat, originLng, item) {
...
function filterAndSortByDistance(items, originLat, originLng, radiusKm) {
  const withDistance = [];
  for (const item of items) {
...
var DEFAULT_NEARBY_RADIUS_KM, MAX_NEARBY_RADIUS_KM, latitudeSchema, longitudeSchema, geoCoordinatesSchema, locationSourceSchema, userLocationUpdateSchema, optionalGeoCoordinatesSchema, nearbyEventsQuerySchema;
var init_geo = __esm({
  "../../packages/shared/src/geo.ts"() {
    "use strict";
    init_zod();
    DEFAULT_NEARBY_RADIUS_KM = 100;
    MAX_NEARBY_RADIUS_KM = 500;
    latitudeSchema = external_exports.number().min(-90, "Latitud inv\xE1lida").max(90, "Latitud inv\xE1lida");
    longitudeSchema = external_exports.number().min(-180, "Longitud inv\xE1lida").max(180, "Longitud inv\xE1lida");
    geoCoordinatesSchema = external_exports.object({
      latitude: latitudeSchema,
      longitude: longitudeSchema
    });
    locationSourceSchema = external_exports.enum(["gps", "manual", "geocode"]);
    userLocationUpdateSchema = external_exports.object({
      latitude: latitudeSchema,
      longitude: longitudeSchema,
      locationSource: locationSourceSchema.optional()
    });
    optionalGeoCoordinatesSchema = external_exports.object({
      ...
      { message: "Indic\xE1 latitud y longitud juntas, o ninguna" }
    );
    nearbyEventsQuerySchema = external_exports.object({
      latitude: external_exports.coerce.number().optional(),
      longitude: external_exports.coerce.number().optional(),
      radiusKm: external_exports.coerce.number().min(1, "Radio m\xEDnimo 1 km").max(MAX_NEARBY_RADIUS_KM, `Radio m\xE1ximo ${MAX_NEARBY_RADIUS_KM} km`).default(DEFAULT_NEARBY_RADIUS_KM)
    });
  }
});

// ../../packages/shared/src/event-location-display.ts
var init_event_location_display = __esm({
  "../../packages/shared/src/event-location-display.ts"() {
    "use strict";
    init_geo();
  }
...
// ../../packages/shared/src/publish-stages.ts
var init_publish_stages = __esm({
  "../../packages/shared/src/publish-stages.ts"() {
    "use strict";
  }
});
...
var registerBase, registerSchema, registerBodySchema, loginSchema, onboardingSchema, preferenciaEventoEnum, tasteOnboardingSchema, userPreferencesPatchSchema, listingInteractionSchema, ticketeraEnum, appBoletosEnum, tipoEntradaEnum, categoriaEventoEnum, listingVisibilitySchema, eventDateSchema, createTicketListingSchema, updateTicketListingSchema, createOrderSchema, confirmReceivedSchema, openDisputeSchema, pixelateRegionSchema, pixelateRegionsSchema;
var init_schemas = __esm({
  "../../packages/shared/src/schemas.ts"() {
    ...
    registerBase = external_exports.object({
      ...
      latitude: external_exports.preprocess(
        (v) => v === "" || v == null || v === void 0 ? void 0 : Number(v),
        latitudeSchema.optional()
      ),
      longitude: external_exports.preprocess(
        (v) => v === "" || v == null || v === void 0 ? void 0 : Number(v),
    ...
    onboardingSchema = external_exports.object({
    ...
    preferenciaEventoEnum = external_exports.enum(["MUSICA", "DEPORTES", "TEATRO", "STAND_UP", "FESTIVALES", "OTRO"]);
    tasteOnboardingSchema = external_exports.object({
      eventPreferences: external_exports.array(preferenciaEventoEnum).min(1, "Eleg\xED al menos un tipo de evento")
    });
    userPreferencesPatchSchema = external_exports.object({
      eventPreferences: external_exports.array(preferenciaEventoEnum).min(1).optional()
    });
    listingInteractionSchema = external_exports.object({
    ...
    ticketeraEnum = external_exports.enum(["TICKETEK", "ALLACCESS", "TICKET_PLUS", "OTRA"]);
    appBoletosEnum = external_exports.enum(["QUENTRO", "ENIGMA", "OTRA"]);
    tipoEntradaEnum = external_exports.enum(["GENERAL", "CAMPO", "PLATEA", "VIP", "OTRO"]);
    categoriaEventoEnum = external_exports.enum(["MUSICA", "DEPORTES", "TEATRO", "FESTIVALES", "OTRO"]);
    listingVisibilitySchema = external_exports.enum(["PUBLIC", "PRIVATE"]);
    eventDateSchema = external_exports.string().min(1, "Fecha del evento requerida").refine((s) => {
      const d = parseDatetimeLocalValue(s) ?? new Date(s);
      return !Number.isNaN(d.getTime());
    }, "Fecha u hora inv\xE1lida");
    createTicketListingSchema = external_exports.object({
      ...
      eventLatitude: external_exports.preprocess(
        (v) => v === "" || v == null || v === void 0 ? void 0 : Number(v),
        latitudeSchema.optional()
      ),
      eventLongitude: external_exports.preprocess(
        (v) => v === "" || v == null || v === void 0 ? void 0 : Number(v),
        longitudeSchema.optional()
      ),
      eventLocationSource: locationSourceSchema.optional()
    });
    updateTicketListingSchema = createTicketListingSchema.partial().extend({
    ...
    pixelateRegionsSchema = external_exports.array(pixelateRegionSchema).optional();
  }
});
...
async function getMarketplaceNearbyRadiusKm() {
  const s = await getPlatformSettings();
...
router.post("/register", async (req, res) => {
  ...
  const userData = {
    ...
    locationUpdatedAt: latitude != null && longitude != null ? /* @__PURE__ */ new Date() : null,
...
router2.put("/location", async (req, res) => {
  const parsed = userLocationUpdateSchema.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: "Coordenadas inv\xE1lidas", details: parsed.error.flatten() });
    return;
  }
...
router2.patch("/profile", async (req, res) => {
  ...
  if (latitude !== void 0 && longitude !== void 0) {
    ...
    if (!Number.isNaN(lat) && !Number.isNaN(lng)) {
      ...
      updateData.locationSource = typeof locationSource === "string" ? locationSource : "manual";
      updateData.locationUpdatedAt = /* @__PURE__ */ new Date();
    }
  }
  if (username !== void 0) {
...
// src/lib/image-sensitive-detection.ts
...
var TEXT_LINE_PAD_FRAC = 0.012;
function clamp012(n) {
  return Math.max(0, Math.min(1, n));
}
function normToPixels(r, W, H) {
  const x = Math.floor(clamp012(r.x) * W);
  const y = Math.floor(clamp012(r.y) * H);
  const w = Math.max(8, Math.floor(clamp012(r.width) * W));
  const h = Math.max(8, Math.floor(clamp012(r.height) * H));
  return {
    x,
    y,
    w: Math.min(W - x, w),
...
function qrLocationToRegion(loc, w, h) {
...
function mergePixelateRegions(regions) {
  let list = regions.map((r) => ({
    x: clamp012(r.x),
    y: clamp012(r.y),
    width: clamp012(r.width),
    height: clamp012(r.height)
  })).filter((r) => r.width >= 8e-3 && r.height >= 8e-3);
  let changed = true;
  while (changed) {
    changed = false;
...
async function detectSensitiveTextRegions(buffer) {
  ...
  return enqueueOcr(async () => {
    ...
    const maxOcr = 2400;
    const scaled = Math.max(img.bitmap.width, img.bitmap.height) > maxOcr ? img.clone().scaleToFit({ w: maxOcr, h: maxOcr }) : img.clone();
    const iw = scaled.bitmap.width;
    const ih = scaled.bitmap.height;
    const jpeg = await scaled.getBuffer(
      "image/jpeg",
      { quality: 88 }
    );
...
// src/lib/ticket-listing-images.ts
async function storeListingCaptureWithRedaction(listingId, kind, file) {
  const ts = Date.now();
...
async function probeCmsSlugs(cmsBase, slugs, download, log2, providerId, eventNameForFilter = "") {
  ...
  for (const slug of filtered.slice(0, 24)) {
    for (const ext of exts) {
      ...
      if (dl.ok) {
        log2(`${providerId} CMS slug OK`, { slug, bytes: dl.buffer.length });
        return url;
      }
    }
  }
...
function titleMatchesEvent(eventName, title) {
  ...
  if (prefixMatch?.[2]) {
    ...
    if (titleNorm.includes("experiencia") || titleNorm.includes("tributo") || titleNorm.includes("homenaje")) {
      return true;
    }
    return titleNorm.includes(artist) && titleNorm.length > artist.length + 6;
  }
  const eventWords = normalizeAscii(eventName).split(/[^a-z0-9]+/).filter((w) => w.length >= 4 && !TITLE_STOPWORDS.has(w));
...
function providersForTicketera(ticketeraId) {
...
var USE_GEMINI = process.env.EVENT_IMAGE_USE_GEMINI === "1" && Boolean(GEMINI_API_KEY);
var ENABLE_AI_GENERATION = process.env.EVENT_IMAGE_AI_GENERATION !== "0";
var MIN_IMAGE_BYTES = Number(process.env.EVENT_IMAGE_MIN_BYTES) || 800;
var LOG_VERBOSE = process.env.EVENT_IMAGE_DEBUG === "1" || process.env.EVENT_IMAGE_DEBUG === "true";
var BLOCKED_HOSTS = [
  "pinterest.",
  "pinimg.",
  "facebook.",
...
function extractHttpsImageUrls(text) {
  const re = /https:\/\/[^\s"'<>)\]]+/gi;
...
async function downloadImageBuffer(url, timeoutMs = 8e3) {
  const host = (() => {
    try {
  ...
  try {
    ...
    if (!ct.startsWith("image/") && ct !== "application/octet-stream") {
      return { ok: false, reason: `content-type no imagen: ${ct || "vac\xEDo"}` };
    }
    const buf = Buffer.from(await res.arrayBuffer());
    if (buf.length < MIN_IMAGE_BYTES) {
      return { ok: false, reason: `muy peque\xF1a (${buf.length} bytes, m\xEDn ${MIN_IMAGE_BYTES})` };
    }
...
async function prepareImageBuffer(buffer) {
  try {
    ...
    if (w < 80 || h < 60) throw new Error(`Imagen demasiado peque\xF1a (${w}x${h})`);
    ...
    if (mime) {
      log("warn", "sharp fall\xF3, subiendo original", {
...
async function searchOfficialImageWithGemini(input) {
  ...
  const prompt = [
    "Encontr\xE1 la URL DIRECTA (HTTPS) de una imagen promocional oficial del evento en Argentina.",
    "Respond\xE9 \xDANICAMENTE con JSON v\xE1lido, sin markdown:",
    ...
    "",
    "La URL debe ser un archivo de imagen (.jpg, .png, .webp), no una p\xE1gina HTML.",
    "Prefer\xED Ticketek, All Access, Movistar Arena, teatro oficial, artista oficial.",
    'Si no hay URL directa fiable, us\xE1 "imageUrl": null.'
  ].join("\n");
...
async function searchWikipediaPageImage(eventName) {
  for (const lang of ["es", "en"]) {
    const apiUrl = `https://${lang}.wikipedia.org/w/api.php?action=query&format=json&origin=*&generator=search&gsrsearch=${encodeURIComponent(eventName)}&gsrlimit=4&prop=pageimages&piprop=thumbnail&pithumbsize=900`;
    ...
    for (const page of Object.values(pages)) {
      ...
      if (!titleMatchesEvent(eventName, title)) {
        log("debug", "wikipedia descartado (t\xEDtulo no coincide)", { eventName, title });
        continue;
      }
...
async function searchItunesArtwork(eventName) {
...
async function searchWikimediaImage(eventName) {
  ...
  const apiUrl = `https://commons.wikimedia.org/w/api.php?action=query&format=json&origin=*&generator=search&gsrsearch=${q}&gsrlimit=8&prop=imageinfo&iiprop=url|thumburl&iiurlwidth=900`;
  const res = await fetch(apiUrl, {
    headers: { "User-Agent": "TicketsTransfer/2.0 EventImageResolver" }
  });
  if (!res.ok) {
  ...
  for (const page of Object.values(pages)) {
    ...
    if (!titleMatchesEvent(eventName, title)) {
      log("debug", "wikimedia descartado (t\xEDtulo no coincide)", { eventName, title });
      continue;
    }
    const info = page.imageinfo?.[0];
    const candidates = [info?.thumburl, info?.url].filter(Boolean);
...
async function trySource(source, url, timeoutMs = 8e3) {
  if (!url) return null;
  ...
  log("warn", `${source} descarga fall\xF3`, { url: url.slice(0, 100), reason: dl.reason });
  return null;
}
async function tryPollinations(input) {
  ...
  return trySource("generated", url, 14e3);
}
async function resolveRemoteImageUrl(input) {
  ...
  log("warn", "todos los pasos fallaron \u2192 fallback categor\xEDa", { fallbackUrl: fallbackUrl.slice(0, 80) });
  const fb = await trySource("fallback", fallbackUrl, 1e4);
  if (fb) return fb;
...
async function resolveAndStoreEventImage(listingId, input) {
  ...
  try {
    ...
    if (!buffer) {
      const dl = await downloadImageBuffer(resolved.url, 1e4);
      if (dl.ok) buffer = dl.buffer;
      else {
        log("warn", "re-descarga fall\xF3", { source: resolved.source, reason: dl.reason });
      }
    }
    if (!buffer) {
      const fallbackUrl = getEventImageCategoryFallback(input.category);
    ...
    const msg = err instanceof Error ? err.message : String(err);
    log("warn", "store excepci\xF3n \u2192 fallback", { listingId, error: msg });
    try {
      const fallbackUrl = getEventImageCategoryFallback(input.category);
      const dl = await downloadImageBuffer(fallbackUrl, 1e4);
      if (dl.ok) {
        const storedUrl = await uploadEventImage(listingId, dl.buffer);
        return { url: storedUrl, source: "fallback" };
      }
    } catch (e2) {
      log("warn", "fallback store fall\xF3", { error: e2 instanceof Error ? e2.message : String(e2) });
    }
    return {
...
var USER_AGENT = "TicketsTransfer/2.0 (geocoding; contacto@soporte-ticketstransfer)";
...
async function resolveEventCoordinates(input) {
  ...
  if (geocoded) {
    return {
      ...
      eventGeocodedAt: /* @__PURE__ */ new Date()
    };
  }
  return {
    eventLatitude: null,
...
var router4 = Router4();
function listingGeoFromDoc(d) {
  return {
    eventLatitude: typeof d.eventLatitude === "number" ? d.eventLatitude : null,
    eventLongitude: typeof d.eventLongitude === "number" ? d.eventLongitude : null,
    eventLocationSource: d.eventLocationSource ?? null
  };
}
var upload2 = multer2({
...
router4.get("/marketplace/nearby", requireAuth, async (req, res) => {
  const configuredRadiusKm = await getMarketplaceNearbyRadiusKm();
  const parsed = nearbyEventsQuerySchema.safeParse({
    ...req.query,
    radiusKm: req.query.radiusKm != null && String(req.query.radiusKm).trim() !== "" ? req.query.radiusKm : configuredRadiusKm
  });
  if (!parsed.success) {
    res.status(400).json({ error: "Par\xE1metros inv\xE1lidos", details: parsed.error.flatten() });
    return;
  }
  ...
  if (!hasValidCoordinates(originLat, originLng)) {
    const userDoc = await db().collection(COLLECTIONS.USERS).doc(req.user.id).get();
    const u = userDoc.data();
    originLat = typeof u?.latitude === "number" ? u.latitude : void 0;
    originLng = typeof u?.longitude === "number" ? u.longitude : void 0;
  }
  if (!hasValidCoordinates(originLat, originLng)) {
    res.status(400).json({
      error: "Configur\xE1 tu ubicaci\xF3n en el registro o perfil, o envi\xE1 latitude y longitude en la consulta.",
...
router4.get("/event-image/preview", requireAuth, async (req, res) => {
  ...
  if (req.query.debug === "1") {
    return res.json({ ...preview, debug: true, hint: "Revis\xE1 logs [event-image] en Vercel" });
  }
  res.json(preview);
});
...
router4.patch("/mine/:listingId", requireAuth, async (req, res) => {
  ...
  if (payload.eventLatitude !== void 0) updates.eventLatitude = payload.eventLatitude ?? null;
  if (payload.eventLongitude !== void 0) updates.eventLongitude = payload.eventLongitude ?? null;
  if (payload.eventLocationSource !== void 0) {
  ...
  const locationFieldsTouched = payload.eventAddress !== void 0 || payload.eventCity !== void 0 || payload.eventPlace !== void 0 || payload.eventLatitude !== void 0 || payload.eventLongitude !== void 0;
  if (locationFieldsTouched) {
    const mergedListing = { ...d, ...updates };
    const eventGeo = await resolveEventCoordinates({
      eventLatitude: mergedListing.eventLatitude,
      eventLongitude: mergedListing.eventLongitude,
      eventLocationSource: mergedListing.eventLocationSource,
      eventAddress: mergedListing.eventAddress ?? null,
      eventCity: mergedListing.eventCity ?? null,
      eventPlace: mergedListing.eventPlace ?? null
    });
  ...
  const orders = await Promise.all(
    snap.docs.map(async (doc) => {
      ...
      return {
        id: doc.id,
        ...d,
        ticketListing: listingDoc.exists ? stripOriginalListingImageUrls({ id: listingDoc.id, ...listingDoc.data() }) : null,
...
router5.get("/:id", async (req, res) => {
  ...
  const buyerDoc = await db().collection(COLLECTIONS.USERS).doc(d.buyerId).get();
  const sellerDoc = await db().collection(COLLECTIONS.USERS).doc(d.sellerId).get();
  const rawListing = listingDoc.exists ? { id: listingDoc.id, ...listingDoc.data() } : null;
  const ticketListingForViewer = rawListing && d.buyerId === req.user.id ? stripOriginalListingImageUrls(rawListing) : rawListing;
  res.json({
    id: doc.id,
    ...d,
...
router6.post("/", async (req, res) => {
  ...
  const ticketListingRaw = listingDoc.exists ? { id: listingDoc.id, ...listingDoc.data() } : null;
  const ticketListingForViewer = ticketListingRaw && order.buyerId === req.user.id ? stripOriginalListingImageUrls(ticketListingRaw) : ticketListingRaw;
  res.status(201).json({
    id: disputeId,
    ...disputeData,
...
router8.get("/ratings", async (req, res) => {
  ...
  if (typeof orderId === "string" && orderId) {
    ...
    const snap = await db().collection(COLLECTIONS.ORDER_RATINGS).orderBy("createdAt", "desc").limit(500).get();
    docs = [...snap.docs];
  }
  const total = docs.length;
  const skip = (pageNum - 1) * limitNum;
  ...
  const ratings = await Promise.all(
    slice.map(async (doc) => {
      ...
      return {
        ...
        positive: !!r.positive,
        points: typeof r.points === "number" ? r.points : r.positive ? PUNTOS_POR_RATING_POSITIVO2 : 0,
...
router8.patch("/ratings/:ratingId", async (req, res) => {
  ...
  if (prevPositive === nextPositive) {
    ...
    return res.json({
      ...
      points: typeof d2.points === "number" ? d2.points : d2.positive ? PUNTOS_POR_RATING_POSITIVO2 : 0,
      createdAt: d2.createdAt?.toDate?.() ?? d2.createdAt
    });
  }
  ...
  if (prevPositive && !nextPositive) {
    const newRep = Math.max(0, currentRep - PUNTOS_POR_RATING_POSITIVO2);
    await ratedRef.update({ reputationScore: newRep, updatedAt: /* @__PURE__ */ new Date() });
  } else if (!prevPositive && nextPositive) {
    await ratedRef.update({
      reputationScore: currentRep + PUNTOS_POR_RATING_POSITIVO2,
      updatedAt: /* @__PURE__ */ new Date()
    });
  }
  await ref.update({
    positive: nextPositive,
    points: nextPositive ? PUNTOS_POR_RATING_POSITIVO2 : 0,
    updatedAt: /* @__PURE__ */ new Date(),
  ...
  res.json({
    ...
    ...d,
    positive: !!d.positive,
    points: typeof d.points === "number" ? d.points : d.positive ? PUNTOS_POR_RATING_POSITIVO2 : 0,
    createdAt: d.createdAt?.toDate?.() ?? d.createdAt
  });
});
router8.delete("/ratings/:ratingId", async (req, res) => {
  ...
  if (r.positive) {
    ...
    if (ratedSnap.exists) {
      ...
      await ratedRef.update({
        reputationScore: Math.max(0, currentRep - PUNTOS_POR_RATING_POSITIVO2),
        updatedAt: /* @__PURE__ */ new Date()
      });
    }
  }
...
router8.patch("/messages/:messageId", async (req, res) => {
  ...
  const updates = { updatedAt: /* @__PURE__ */ new Date(), moderatedAt: /* @__PURE__ */ new Date(), moderatedByUserId: req.user.id };
  if (body.redact === true) {
    updates.content = REDACTED_MESSAGE;
    updates.redactedByAdmin = true;
  } else if (typeof body.content === "string" && body.content.trim()) {
    const text = body.content.trim().slice(0, 2e3);
    updates.content = text;
    updates.editedByAdmin = true;
  } else {
    return res.status(400).json({ error: "Envi\xE1 content (texto) o redact: true" });
  }
...
router8.patch("/dispute-messages/:messageId", async (req, res) => {
  ...
  if (body.redact === true) {
    ...
    updates.content = body.content.trim().slice(0, 2e3);
    updates.editedByAdmin = true;
  } else {
    return res.status(400).json({ error: "Envi\xE1 content (texto) o redact: true" });
  }
...
router8.patch("/disputes/:disputeId/notes", async (req, res) => {
  ...
  await ref.update({ adminNotes, updatedAt: /* @__PURE__ */ new Date() });
  const d = (await ref.get()).data();
  res.json({
    id: disputeId,
...
settingsRouter.get("/branding", async (_req, res) => {
  ...
  const pickStr = (k) => typeof users[k] === "string" ? users[k] : void 0;
  res.json({
...
settingsRouter.get("/marketplace-nearby", requireAuth, async (_req, res) => {
  const nearbyRadiusKm = await getMarketplaceNearbyRadiusKm();
